...
class RubiksCube:
    ...
    def __init__(self):
        self.current_state = self.SOLVED_STATE.copy()
    ...
    def solve_cube(self) -> tuple:
        scrambled_state = self._find_correct_orientation(self.current_state)

        scrambled_graph = nx.Graph()
        solved_graph = nx.Graph()

        scrambled_queue = [scrambled_state]
        scrambled_graph.add_node(tuple(scrambled_state))
